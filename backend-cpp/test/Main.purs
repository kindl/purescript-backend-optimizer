...
import PureScript.Backend.Optimizer.Codegen.Cpp (esCodegenModule)
import PureScript.Backend.Optimizer.Codegen.Cpp.Builder (coreFnModulesFromOutput)
...
runSnapshotTests { accept, filter } = do
  liftEffect $ Process.chdir $ Path.concat [ "backend-cpp", "test", "snapshots" ]
  spawnFromParent "spago.cmd" [ "build", "-u", "-g corefn" ]
  ...
  coreFnModulesFromOutput "output" >>= case _ of
    ...
    Right coreFnModules -> do
      ...
      results <- forWithIndex outputModules \name output -> do
        let snapshotFilePath = Path.concat [ "..", "snapshots-out", name <> ".cpp" ]
