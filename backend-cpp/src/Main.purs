...
import PureScript.Backend.Optimizer.Codegen.Cpp (esCodegenModule, esModulePath)
import PureScript.Backend.Optimizer.Codegen.Cpp.Builder (basicBuildMain, externalDirectivesFromFile)
import PureScript.Backend.Optimizer.Codegen.Cpp.Foreign (esForeignSemantics)
  ...
  ArgParser.fromRecord
    ...
    , outputDir:
        ArgParser.argument [ "--output-dir" ]
          ...
          \Defaults to './output-cpp'."
          # ArgParser.default (Path.concat [ ".", "output-cpp" ])
    , foreignDir:
  ...
  ArgParser.fromRecord
    ...
    , targetFile:
        ArgParser.argument [ "-t", "--to" ]
          ...
          \Defaults to './index.cpp'."
          # ArgParser.default "index.cpp"
    , targetPlatform:
...
esArgParser =
  ArgParser.choose "command"
    ...
    <* ArgParser.flagHelp
    <* ArgParser.flagInfo [ "--version", "-v" ] "Show the current version of purs-backend-cpp." Version.version

parseArgs :: Effect (Either ArgParser.ArgError Command)
parseArgs = do
  cliArgs <- Array.drop 2 <$> Process.argv
  pure $ ArgParser.parseArgs "purs-backend-cpp"
...
main cliRoot =
  ...
  buildCmd args = basicBuildMain
    ...
    , onCodegenBefore: do
        ...
        copyFile (Path.concat [ cliRoot, "runtime.cpp" ]) (Path.concat [ args.outputDir, "runtime.cpp" ])
    ...
    , onCodegenModule: \build (Module coreFnMod) backendMod@{ name: ModuleName name } -> do
        ...
        mkdirp modPath
        writeTextFile UTF8 (Path.concat [ modPath, "index.cpp" ]) formatted
        unless (Set.isEmpty backendMod.foreign) do
          let foreignOutputPath = Path.concat [ modPath, "foreign.cpp" ]
          origPath <- liftEffect $ Path.resolve [ args.outputDir, ".." ] coreFnMod.path
          let foreignSiblingPath = fromMaybe origPath (String.stripSuffix (Pattern (Path.extname origPath)) origPath) <> ".cpp"
          res <- attempt $ oneOf
