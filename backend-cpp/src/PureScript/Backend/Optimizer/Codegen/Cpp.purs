module PureScript.Backend.Optimizer.Codegen.Cpp where
...
import PureScript.Backend.Optimizer.Codegen.Cpp.Common (CppStatement(..), esAccessor, esApp, esArray, esAssign, esAssignRef, esBinding, esBindings, esBlock, esBlockStatements, esBoolean, esBranches, esChar, esComment, esContinue, esCurriedFn, esEffectBlock, esError, esEscapeSpecial, esExportAllFrom, esExports, esFn, esIdent, esImport, esImports, esIndex, esInt, esLetBindings, esModuleName, esNumber, esOffset, esProp, esPure, esRecord, esReturn, esSepStatements, esString, esUndefined, esUpdateLeft)
import PureScript.Backend.Optimizer.Codegen.Cpp.Inline (esInlineMap)
...
esCodegenModule options implementations mod = do
  let
    ...
    header :: Array (CppStatement (Dodo.Doc a))
    header = fold
      [ [ Statement (esModuleHeader mod.name), Statement esImportRuntime ]
      , (\mn -> Statement (esImport mn (esModulePath mn))) <$> Set.toUnfoldable mod.imports
      , Monoid.guard (not (Set.isEmpty mod.foreign)) [ Statement (esImports (esForeignModuleName mod.name) foreignImports) ]
      ]

    statements :: Array (CppStatement (Dodo.Doc a))
    statements = fold
...
--    , map (Statement <<< uncurry (maybe (esExports Nothing) (esExports <<< Just)) <<< map (Array.sortBy (comparing fst))) $ Map.toUnfoldable $ unwrap exportsByPath
    ...
    footer :: Array (CppStatement (Dodo.Doc a))
    footer = Monoid.guard (not (Set.isEmpty mod.foreign)) [ Statement (esExportAllFrom (esForeignModuleName mod.name)) ]

  Monoid.guard (not (Array.null mod.comments))
    ( Dodo.lines (esComment <$> mod.comments)
        <> Dodo.break
    )
    <> esBlockStatements header
    <> Dodo.break
    <> Dodo.text "export"
    <> Dodo.space
    <> Dodo.text "namespace"
    <> Dodo.space
    <> esModuleName mod.name
    <> Dodo.space
    <> Dodo.Common.jsCurlies (esBlockStatements statements)
    <> Dodo.break
    <> esBlockStatements footer
    <> Dodo.break
...
shouldInlineApp :: forall a. CodegenEnv -> Qualified Ident -> Array TcoExpr -> Maybe (Tuple (Dodo.Doc a) (CppStatement (Dodo.Doc a)))
...
esCodegenBlockStatements :: forall a. BlockMode -> CodegenEnv -> TcoExpr -> Array (CppStatement (Dodo.Doc a))
...
esCodegenBlockReturn :: forall a. BlockMode -> CodegenEnv -> TcoExpr -> Array (CppStatement (Dodo.Doc a))
...
esCodegenBlockBranches mode env bs def = esBranches (go <$> bs) (esCodegenBlockStatements mode env <$> def)
  where
  go :: Pair TcoExpr -> Tuple (Dodo.Doc a) (Array (CppStatement (Dodo.Doc a)))
...
esCodegenTcoJump :: forall a. BlockMode -> TcoPop -> Array (Dodo.Doc a) -> Array (CppStatement (Dodo.Doc a))
...
esCodegenTcoReturn :: forall a. BlockMode -> Tuple Ident (List Ident) -> Dodo.Doc a -> Array (CppStatement (Dodo.Doc a))
...
esCodegenTcoJoin :: forall a. BlockMode -> Dodo.Doc a -> Array (CppStatement (Dodo.Doc a))
...
esModulePath (ModuleName mn) = "../" <> mn <> "/index.cpp"

esForeignModulePath :: ModuleName -> String
esForeignModulePath (ModuleName _) = "./foreign.cpp"

esForeignModuleName :: ModuleName -> String
esForeignModuleName (ModuleName _) = ":foreign"
...
esTcoFnBody :: forall a. Ident -> NonEmptyArray Ident -> Dodo.Doc a -> Array (CppStatement (Dodo.Doc a))
...
esTopLevelLazyBinding :: forall a. Ident -> Array (CppStatement (Dodo.Doc a)) -> Dodo.Doc a
esTopLevelLazyBinding ident bs = esBinding ident $ esPure $ esApp (Dodo.text "runtime::binding") [ esFn [] bs ]

esLazyBinding :: forall a. Ident -> Array (CppStatement (Dodo.Doc a)) -> Dodo.Doc a
...
esModuleHeader modName = Dodo.words
  [ Dodo.text "export"
  , Dodo.text "module"
  , esModuleName modName
  ]
